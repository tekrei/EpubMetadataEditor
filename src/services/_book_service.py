...
class BookService:
    ...
    def get_metadata(self, path: str | Path) -> dict[str, any]:
        ...
        try:
            with zipfile.ZipFile(path, "r") as zin:
                ...
                identifier_elems = opf_tree.xpath(
                    ".//dc:identifier/text()", namespaces=NAMESPACES
                )

                metadata["title"] = self._get_text_or_default(
                    title_elem, "Unknown Title"
                )
                metadata["author"] = self._get_text_or_default(
                    author_elem, "Unknown Author"
                )
        ...
        try:
            with zipfile.ZipFile(path, "r") as zin:
                ...
                with zipfile.ZipFile(temp_path, "w") as zout:
                    ...
                    new_opf_tree, internal_img_update = process_func(
                        opf_tree, rootfile_path
                    )
                    ...
                    for item in zin.infolist():
                        ...
                        if item.filename == rootfile_path:
                            zout.writestr(
                                item.filename,
                                etree.tostring(
                                    new_opf_tree, encoding="utf-8", xml_declaration=True
                                ),
                                compress_type=zipfile.ZIP_DEFLATED,
                            )
                        elif (
                            internal_img_update
                            and item.filename == internal_img_update["path"]
                        ):
                            if internal_img_update["new_data"] is not None:
                                zout.writestr(
                                    item.filename,
                                    internal_img_update["new_data"],
                                    compress_type=zipfile.ZIP_DEFLATED,
                                )
        ...
        if elem is not None:
            ...
            if metadata_tag is not None:
                new_elem = etree.SubElement(
                    metadata_tag, f"{{{NAMESPACES['dc']}}}{tag_name}"
                )
                new_elem.text = value
    ...
    def _set_identifier_tag(self, opf_tree, isbn: str):
        ...
        for node in ids:
            if node.text and (
                "isbn" in node.text.lower()
                or len("".join(filter(str.isdigit, node.text))) in (10, 13)
            ):
        ...
        if not found:
            ...
            if metadata_tag is not None:
                new_elem = etree.SubElement(
                    metadata_tag, f"{{{NAMESPACES['dc']}}}identifier"
                )
            ...
            with zipfile.ZipFile(path, "r") as zin:
                ...
                if cover_href:
                    image_path = (
                        cover_href
                        if root_dir == "."
                        else str(Path(root_dir) / cover_href)
                    )
    ...
    def _find_cover_href(self, opf_tree) -> str | None:
        ...
        # EPUB 3
        cover_items = opf_tree.xpath(
            '//opf:item[contains(@properties, "cover-image")]/@href',
        ...
        cover_id = opf_tree.xpath(
            '//opf:meta[@name="cover"]/@content', namespaces=NAMESPACES
        )
        if cover_id:
            cover_node = opf_tree.xpath(
                f'//opf:item[@id="{cover_id[0]}"]/@href', namespaces=NAMESPACES
            )
            if cover_node:
                return cover_node[0]
        return None
    ...
    def update_cover(self, path: str | Path, image_path: str | None):
        ...
        def update_logic(opf_tree, rootfile_path):
            ...
            if cover_href:
                full_img_path = (
                    root_dir / cover_href if str(root_dir) != "." else Path(cover_href)
                )
            ...
            if image_path:
                ...
                for node in opf_tree.xpath(
                    '//opf:item[contains(@properties, "cover-image")]',
                    namespaces=NAMESPACES,
                ):
                    node.getparent().remove(node)
                for node in opf_tree.xpath(
                    '//opf:meta[@name="cover"]', namespaces=NAMESPACES
                ):
                    node.getparent().remove(node)
    ...
    def fetch_metadata_by_isbn(
        self, isbn: str, provider: str = "google"
    ) -> dict[str, any]:
    ...
    def _fetch_google(self, isbn: str) -> dict[str, any]:
        ...
        try:
            ...
            if e.code in (503, 504, 429):
                raise NetworkError(
                    f"Google Books service is temporarily unavailable (HTTP {e.code})"
                ) from e
    ...
    def _fetch_openlibrary(self, isbn: str) -> dict[str, any]:
        ...
        try:
            with urllib.request.urlopen(url, timeout=10) as response:
                ...
                if key in data:
                    ...
                    return {
                        ...
                        "publisher": ", ".join(
                            [p.get("name", "") for p in info.get("publishers", [])]
                        ),
            ...
            if e.code in (503, 504, 429):
                raise NetworkError(
                    f"Open Library service is temporarily unavailable (HTTP {e.code})"
                ) from e
